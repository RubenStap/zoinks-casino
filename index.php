...
<body>
    ...
    <main>
        <section>
            <div class="container">
                <div class="form-container">
                    <form method="post"> <!--form = Formulier -->
                        ...
                        <button type="input" class="button">Controleren</button>
                        <!--Dit is de controleer button waarin de functie uitgevoerd wordt die gemaakt is.-->
                    </form>
                </div>
            </div>
            <?php
            if (isset($_POST['age'])) { //isset = kijkt of er een waarde is
                if ($_POST['age'] < 16) { // < 16 = jonger dan 16 + je bent niet welkom op de pagina
                    if (!isset($_SESSION['age'])) { 
                        $_SESSION['age'] = $_POST['age']; // Wordt in een session['age'] gezet.
                    }
                } elseif ($_POST['age'] >= 16) { // >= 16 = ouder dan 16 + je bent welkom op de pagina
            ...
            if(isset($_SESSION['age'])) {
                if ($_SESSION['age'] >= 16) {
                    echo "<script>alert('Je bent ouder dan 16. Je bent welkom in het zoinks casino.')</script>";
                }}
            if (isset($_SESSION['age'])) {
                if ($_SESSION['age'] < 16) {
                    echo "<script>alert('Je bent jonger dan 16. Je bent niet welkom.')</script>";                
                }
            }
